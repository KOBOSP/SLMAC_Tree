 ...
 * @return int                          成功匹配的数目
 */
int ORBmatcher::SearchFMatchPointByProjectMapPoint(Frame &F, const vector<MapPoint*> &vpMapPoints, bool bFactor, const float th)
    {
        int nmatches=0;
        // Step 1 遍历有效的局部地图点
 ...
 */
int ORBmatcher::SearchKFNewMatchPointBySim3andLocalMP(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th, bool NeedObjectMP)
{
 ...
 * @return int                      成功匹配的数量
 */
int ORBmatcher::SearchFMatchPointByProjectLastFrame(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
    {
        int nmatches = 0;
        ...
        for(int i=0; i<LastFrame.mnKeyPointNum; i++){
            ...
            if(pMP){
                ...
                if(!LastFrame.mvbOutlier[i]){
                    ...
                    vIndices2 = CurrentFrame.GetFeaturesInArea(u,v, radius, false,nLastOctave-1, nLastOctave+1);
 ...
 * @return                    成功匹配的数量
 */
int ORBmatcher::SearchKFMatchPointByKFBoW(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches12, bool NeedObjectMP){
    ...
    const vector<MapPoint*> vpMapPoints2 = pKF2->GetAllMapPointVectorInKF();
...
int ORBmatcher::SearchNewKFMatchPointByKFF12(KeyFrame *pKF1, KeyFrame *pKF2, cv::Mat F12,
 ...
 */
int ORBmatcher::SearchNewKFMatchPointByKFMutualSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint*> &vpMatches12,
                                                    const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th, bool NeedObjectMP)
    {
        ...
        // 取出关键帧中的地图点
        const vector<MapPoint*> vpMapPoints1 = pKF1->GetAllMapPointVectorInKF(NeedObjectMP);
        const int N1 = vpMapPoints1.size();

        const vector<MapPoint*> vpMapPoints2 = pKF2->GetAllMapPointVectorInKF(NeedObjectMP);
        ...
        for(int i1=0; i1<N1; i1++){
            ...
            const vector<size_t> vIndices = pKF2->GetKeyPointsByArea(u, v, radius);
            if(vIndices.empty())
                continue;
            // Match to the most similar keypoint in the radius
            const cv::Mat dMP = pMP->GetDescriptor();
        ...
        for(int i2=0; i2<N2; i2++){
            ...
            const vector<size_t> vIndices = pKF1->GetKeyPointsByArea(u, v, radius);
            if(vIndices.empty())
                continue;
            // Match to the most similar keypoint in the radius
            const cv::Mat dMP = pMP->GetDescriptor();
 ...
 */
int ORBmatcher::SearchAndFuseMPsInLoopClose(KeyFrame *pKF, cv::Mat Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
{
    // Get Calibration Parameters for later projection
    const float &fx = pKF->fx;
    const float &fy = pKF->fy;
    ...
    // Set of MapPoints already found in the KeyFrame
    // 当前帧已有的匹配地图点
    const set<MapPoint*> spAlreadyFound = pKF->GetAllMapPointSetInKF();
...
*/
int ORBmatcher::FuseRedundantMapPointAndSameIdObjectInLocalMap(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th, bool bInLoopClose)
{
    ...
    for(int i=0; i<nMPs; i++){
        ...
        if(pMP->mnObjectID<0){
            ...
            if(bInLoopClose==false && (dist3D<minDistance || dist3D>maxDistance))
                continue;
            // Viewing angle must be less than 60 deg
            // Step 4 地图点到光心的连线与该地图点的平均观测向量之间夹角要小于60°
            cv::Mat Pn = pMP->GetNormal();
            if(bInLoopClose==false && PO.dot(Pn)<0.5*dist3D)
                continue;
            ...
            for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)// 步骤3：遍历搜索范围内的features
            {
                ...
                if(bInLoopClose==false && (kpLevel<nPredictedLevel-1 || kpLevel>nPredictedLevel))
                    continue;
                ...
                if(bInLoopClose==false && (e2*pKF->mvInvLevelSigma2[kpLevel]>5.99))
                    continue;
                const cv::Mat &dKF = pKF->mDescriptors.row(idx);
                const int dist = ComputeDescriptorDistance(dMP, dKF);
                // 和投影点的描述子距离最小
        ...
        // 最佳匹配距离要小于阈值
        if((bInLoopClose==false && bestDist<=TH_LOW)||(bInLoopClose == true && bestDist<=TH_HIGH)){
            MapPoint* pMPinKF = pKF->GetMapPointByIndex(bestIdx);
            if(pMPinKF){
