 ...
 */
                                                ...
                                                vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
    {
        ...
        // Launch threads to compute in parallel a fundamental matrix and a homography matrix
        // Step 3 计算fundamental 矩阵 和homography 矩阵，为了加速分别开了线程计算

        //这两个变量用于标记在H和F的计算中哪些特征点对被认为是Inlier
        vector<bool> vbMatchesInliersH, vbMatchesInliersF;
        //计算出来的单应矩阵和基础矩阵的RANSAC评分，这里其实是采用重投影误差来计算的
        float ScoreH, ScoreF; //score for MatH and MatF
        //这两个是经过RANSAC算法后计算出来的单应矩阵和基础矩阵
        cv::Mat MatH, MatF;
/////////2021.9.22.20.36
        // 构造线程来计算H矩阵及其得分
        // thread方法比较特殊，在传递引用的时候，外层需要用ref来进行引用传递，否则就是浅拷贝
        thread threadH(&Initializer::GetHomographyMatrix,	//该线程的主函数
                       this,							//由于主函数为类的成员函数，所以第一个参数就应该是当前对象的this指针
                       ref(vbMatchesInliersH), 			//输出，特征点对的Inlier标记
                       ref(ScoreH), 						//输出，计算的单应矩阵的RANSAC评分
                       ref(MatH));						//输出，计算的单应矩阵结果
        // 计算fundamental matrix并打分，参数定义和H是一样的，这里不再赘述
        thread threadF(&Initializer::GetFundamentalMatrix,
                       this,
        ...
        float RatioH = ScoreH / (ScoreH + ScoreF);			//RatioH=Ratio of Homography

        // Try to reconstruct from homography or fundamental depending on the ratio (0.40-0.45)
        // 注意这里更倾向于用H矩阵恢复位姿。如果单应矩阵的评分占比达到了0.4以上,则从单应矩阵恢复运动,否则从基础矩阵恢复运动
        if(RatioH > 0.40)
            //更偏向于平面，此时从单应矩阵恢复，函数ReconstructH返回bool型结果
            return ReconstructFromH(vbMatchesInliersH,    //输入，匹配成功的特征点对Inliers标记
                                    MatH,                    //输入，前面RANSAC计算后的单应矩阵
                                    mK,                    //输入，相机的内参数矩阵
            ...
            // 更偏向于非平面，从基础矩阵恢复
            return ReconstructFromF(vbMatchesInliersF, MatF, mK, R21, t21, vP3D, vbTriangulated, 1.0, 50);
    }
 ...
 * @param[in & out] MatchesScore                     计算单应矩阵的得分
 * @param[in & out] H21                       单应矩阵结果
 */
    void Initializer::GetHomographyMatrix(vector<bool> &vbMatchesInliers, float &MatchesScore, cv::Mat &H21)
    {
        const int N = mvMatches12.size();

        // NormalizePointCoord coordinates
        ...
        // 记录各自的归一化矩阵
        cv::Mat T1, T2;
        NormalizePointCoord(mvKeys1, vPn1, T1);
        NormalizePointCoord(mvKeys2, vPn2, T2);
        ...
        // Perform all RANSAC iterations and save the solution with highest MatchesScore
        //下面进行每次的RANSAC迭代
        for(int it=0; it<mMaxIterations; it++){
            // Select a minimum set
            // Step 2 选择8个归一化之后的点对进行迭代
            ...
            // Step 4 利用重投影误差为当次RANSAC的结果评分
            CurrentScore = GetHomographyScore(H21i, H12i,            //输入，单应矩阵的计算结果
                                              vbCurrentInliers,    //输出，特征点对的Inliers标记
                                              mSigma);				//TODO  测量误差，在Initializer类对象构造的时候，由外部给定的


            // Step 5 更新具有最优评分的单应矩阵计算结果,并且保存所对应的特征点对的内点标记
            if(CurrentScore > MatchesScore)
            {
 ...
 *
 * @param[in & out] vbInliers          标记是否是外点
 * @param[in & out] score                     计算基础矩阵得分
 * @param[in & out] F21                       从特征点1到2的基础矩阵
 */
    void Initializer::GetFundamentalMatrix(vector<bool> &vbInliers, float &score, cv::Mat &F21)
    {
        // 计算基础矩阵,其过程和上面的计算单应矩阵的过程十分相似.

        // Number of putative matches
        // 匹配的特征点对总数
        // const int mnKeyPointNum = vbMatchesInliers.size();  // !bug！请使用下面代替
        const int N = mvMatches12.size();
        // NormalizePointCoord coordinates
        // Step 1 将当前帧和参考帧中的特征点坐标进行归一化，主要是平移和尺度变换
        // 具体来说,就是将mvKeys1和mvKey2归一化到均值为0，一阶绝对矩为1，归一化矩阵分别为T1、T2
        ...
        for(int it=0; it<mMaxIterations; it++)
        {
            ...
            // Step 4 利用重投影误差为当次RANSAC的结果评分
            currentScore = GetFundamentalScore(F21i, vbCurrentInliers, mSigma);
 ...
 */
            ...
            const vector<cv::Point2f> &vP2) //归一化后的点, in current frame
    {
        ...
        //初始化A矩阵
        cv::Mat A(N,9,CV_32F); // mnKeyPointNum*9维
 ...
 * @return float                        返回得分
 */
    float Initializer::GetHomographyScore(
 ...
 * @return float                        返回得分
 */
    float Initializer::GetFundamentalScore(
 ...
 * @param[in] minTriangulAngle          认为三角化有效的最小视差角
 * @param[in] minTriangulatedMapPoint   最小三角化点数量
 * @return true                         成功初始化
 * @return false                        初始化失败
 */
    bool Initializer::ReconstructFromF(vector<bool> &vbMatchesInliers, cv::Mat &F21, cv::Mat &K,
                                       cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minTriangulAngle, int minTriangulatedMapPoint)
    {
        // Step 1 统计有效匹配点个数，并用 mnKeyPointNum 表示
        ...
        // 实现：根据计算的解组合成为四种情况,并依次调用 Initializer::GetTriangulFromRT() 进行检查,得到可以进行三角化测量的点的数目
        // 定义四组解分别在对同一匹配点集进行三角化测量之后的特征点空间坐标
        vector<vector<cv::Point3f> > vvP3D(4);
        vector<vector<bool> > vvbTriangulated(4);
        vector<float> vfTriangulAngle(4);
        vector<int> nGood(4);
        int maxGoodIdx=-1;
// Step 4.1 使用同样的匹配点分别检查四组解，记录当前计算的3D点在摄像头前方且投影误差小于阈值的个数，记为有效3D点个数
//        int nGood1 = GetTriangulFromRT(R1, t1,                            //当前组解
...
//                                       TriangulAngle1);					//认为某对特征点三角化测量有效的比较大的视差角
        for(int i=0;i<4;i++){
            nGood[i]=GetTriangulFromRT(R2, t1, mvKeys1, mvKeys2,
                                                    mvMatches12, vbMatchesInliers, K, vvP3D[i], 4.0 * mSigma2,
                                       vvbTriangulated[i], vfTriangulAngle[i]);
            if(nGood[i]>maxGoodIdx){
                maxGoodIdx=i;
            }
        }
        ...
        // 条件2: 三角化视差角 parallax 必须大于最小视差角 minTriangulAngle，角度越大3D点越稳定
        R21 = cv::Mat();
        t21 = cv::Mat();
        int nMinGood = max(static_cast<int>(0.9*N), minTriangulatedMapPoint);
        int nsimilar = 0;
        for(int i=0;i<4;i++) {
            if(nGood[i]>0.7*nGood[maxGoodIdx]){
                nsimilar++;
            }
        }
        if(nGood[maxGoodIdx]<nMinGood || nsimilar>1 || vfTriangulAngle[maxGoodIdx] < minTriangulAngle){
            return false;
        }else{
            // 存储3D坐标
            vP3D = vvP3D[maxGoodIdx];
            vbTriangulated = vvbTriangulated[maxGoodIdx];
            R1.copyTo(R21);
            t1.copyTo(t21);
            return true;
        }
 ...
 * @return false                        初始化失败
 */
    bool Initializer::ReconstructFromH(vector<bool> &vbMatchesInliers, cv::Mat &H21, cv::Mat &K,
                                       cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
    {
        ...
        for(size_t i=0; i<8; i++){
            ...
            // 调用 Initializer::GetTriangulFromRT(), 计算good点的数目
            int nGood = GetTriangulFromRT(vR[i], vt[i],                    //当前组解的旋转矩阵和平移向量
                                          mvKeys1, mvKeys2,                //特征点
 ...
 */
    void Initializer::GetTriangulMapPoint(
 ...
 */
    void Initializer::NormalizePointCoord(const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T)//将特征点归一化的矩阵
    {
 ...
 * @param[in & out] TriangulAngle                   计算出来的比较大的视差角（注意不是最大，具体看后面代码）
 * @return int
 */
    int Initializer::GetTriangulFromRT(const cv::Mat &R, const cv::Mat &t, const vector<cv::KeyPoint> &vKeys1, const vector<cv::KeyPoint> &vKeys2,
                                       const vector<Match> &vMatches12, vector<bool> &vbMatchesInliers,
                                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &TriangulAngle)
    {
        ...
        for(size_t i=0, iend=vMatches12.size();i<iend;i++){
            ...
            // 利用三角法恢复三维点p3dC1
            GetTriangulMapPoint(kp1, kp2,    //特征点
                                P1, P2,        //投影矩阵
            ...
            // Check depth in front of first camera (only if enough TriangulAngle, as "infinite" points can easily go to negative depth)
            // 如果深度值为负值，为非法三维点跳过该匹配点对
            // ?视差比较小时，重投影误差比较大。这里0.99998 对应的角度为0.36237°,这里不应该是 cosParallax>0.99998 吗？
            // ?因为后面判断vbGood 点时的条件也是 cosParallax<0.99998
            // !可能导致初始化不稳定
            if(p3dC1.at<float>(2)<=0 || cosParallax>0.99998)
                continue;

            // Check depth in front of second camera (only if enough TriangulAngle, as "infinite" points can easily go to negative depth)
        ...
        if(nGood>0){
            ...
            //将这个选中的角弧度制转换为角度制
            TriangulAngle = acos(vCosParallax[idx]) * 180 / CV_PI;
        }
        else{
