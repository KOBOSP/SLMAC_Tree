...
// 构造函数
    KeyFrameDatabase::KeyFrameDatabase(const ORBVocabulary &voc, const string &strSettingPath) :
            mpVoc(&voc) {
        cv::FileStorage fSettings(strSettingPath, cv::FileStorage::READ);
        float radioCommonWord=fSettings["KFDB.radioCommonWord"];
        float radioAccScore=fSettings["KFDB.radioAccScore"];
        mfradioCommonWord=radioCommonWord;
        mfradioAccScore=radioAccScore;
        // 数据库的主要内容了
        mvKFListHasWord.resize(voc.size()); // number of words
        cout << "- KFDB.radioCommonWord: " << mfradioCommonWord << endl;
        cout << "- KFDB.radioAccScore: " << mfradioAccScore << endl;
    }
 ...
 */
    void KeyFrameDatabase::addKFtoDB(KeyFrame *pKF) {
        // 线程锁
 ...
 * @param[in] pKF   删除的关键帧
 */
    void KeyFrameDatabase::eraseKFromDB(KeyFrame *pKF) {
        // 线程锁，保护共享数据
        unique_lock<mutex> lock(mMutex);
...
// 清空关键帧数据库
    void KeyFrameDatabase::clearAllKFinDB() {
        mvKFListHasWord.clear();// mvKFListHasWord[i]表示包含了第i个word id的所有关键帧
        mvKFListHasWord.resize(mpVoc->size());// mpVoc：预先训练好的词典
    }
 ...
 */
    vector<KeyFrame *> KeyFrameDatabase::DetectLoopClosingCandidates(KeyFrame *pKF, float minScore) {
        // this KF share same MP with pKF, so they are neighbor
        // 取出与当前关键帧相连（>15个共视地图点）的所有关键帧，这些相连关键帧都是局部相连，在闭环检测的时候将被剔除
        // 相连关键帧定义见 KeyFrame::UpdateConnectedKeyFrameAndWeights()
        set<KeyFrame *> spConnectedKeyFrames = pKF->GetCovisibleKeyFrames();

        // 用于保存可能与当前关键帧形成闭环的候选帧（只要有相同的word，且不属于局部相连（共视）帧）
        list<KeyFrame *> lKFsShareWord;
        ...
        for (list<KeyFrame *>::iterator lit = lKFsShareWord.begin(), lend = lKFsShareWord.end();
             lit != lend; lit++) {
            if ((*lit)->mnShareWord > maxCommonWords)
                maxCommonWords = (*lit)->mnShareWord;
        }

        // 确定最小公共单词数为最大公共单词数目的0.8倍
        int minCommonWords = maxCommonWords * mfradioCommonWord;;
        ...
        for (list<KeyFrame *>::iterator lit = lKFsShareWord.begin(), lend = lKFsShareWord.end();
            ...
            if (pKFi->mnShareWord > minCommonWords) {
                // 用mBowVec来计算两者的相似度得分
        ...
        for (list<pair<float, KeyFrame *> >::iterator it = lScoreAndMatch.begin(), itend = lScoreAndMatch.end();
            ...
            vector<KeyFrame *> vpNeighs = pKFi->GetCovisibilityForefrontKeyFrames(10);
        ...
        float minScoreToRetain = bestAccScore * mfradioAccScore;
