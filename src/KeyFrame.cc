...
// 判断某个点是否在当前关键帧的图像中
    ...
    cv::Point2f KeyFrame::ProjectMPToKP(const cv::Point3f &p3D) {
        return cv::Point2f(fx * p3D.x / p3D.z + cx,
                           fy * p3D.y / p3D.z + cy);
    }
    Eigen::Vector2d KeyFrame::ProjectMPToKP(const Eigen::Vector3d &v3D) {
        Eigen::Vector2d res;
        res[0] = fx * v3D[0] / v3D[2] + cx;
        res[1] = fy * v3D[1] / v3D[2] + cy;
        return res;
    }
    Eigen::Vector2f KeyFrame::ProjectMPToKP(const Eigen::Vector3f &v3D) {
        Eigen::Vector2f res;
        res[0] = fx * v3D[0] / v3D[2] + cx;
        res[1] = fy * v3D[1] / v3D[2] + cy;
        return res;
