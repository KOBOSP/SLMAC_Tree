...
{
    {
        ...
        if(!mmConnectedKeyFrameandWeights.count(pKF))
            ...
            mmConnectedKeyFrameandWeights[pKF]=weight;
        else if(mmConnectedKeyFrameandWeights[pKF] != weight)
            // 之前连接的权重不一样了，需要更新
            mmConnectedKeyFrameandWeights[pKF]=weight;
        else
            return;
    ...
    UpdateCovisibleKeyFrameandWeights();
}
 ...
 */
void KeyFrame::UpdateCovisibleKeyFrameandWeights(){
    // 互斥锁，防止同时操作共享数据产生冲突
    unique_lock<mutex> lock(mMutexConnections);
    ...
    vPairs.reserve(mmConnectedKeyFrameandWeights.size());
    // 取出所有连接的关键帧，mConnectedKeyFrameWeights的类型为std::map<KeyFrame*,int>，而vPairs变量将共视的地图点数放在前面，利于排序
    for(map<KeyFrame*,int>::iterator mit=mmConnectedKeyFrameandWeights.begin(), mend=mmConnectedKeyFrameandWeights.end(); mit != mend; mit++)
    ...
    mvOrderedConnectedWeights = vector<int>(lWs.begin(), lWs.end());
}

// 得到与该关键帧连接（>15个共视地图点）的关键帧(没有排序的)
set<KeyFrame*> KeyFrame::GetCovisibleKeyFrames(){
    unique_lock<mutex> lock(mMutexConnections);
    set<KeyFrame*> s;
    for(map<KeyFrame*,int>::iterator mit=mmConnectedKeyFrameandWeights.begin(); mit != mmConnectedKeyFrameandWeights.end(); mit++){
        s.insert(mit->first);
    }
    return s;
}
 ...
 */
vector<KeyFrame*> KeyFrame::GetCovisibilityForefrontKeyFrames(const int &N){
    unique_lock<mutex> lock(mMutexConnections);
...
vector<KeyFrame*> KeyFrame::GetCovisiblesByWeight(const int &w){
    ...
    vector<int>::iterator it = upper_bound(mvOrderedConnectedWeights.begin(),   //起点
                                            mvOrderedConnectedWeights.end(),     //终点
                                            w,                          //目标阈值
                                            KeyFrame::weightComp);      //比较函数从大到小排序
    // 如果没有找到，说明最大的权重也比给定的阈值小，返回空
    if(it == mvOrderedConnectedWeights.end() && *mvOrderedConnectedWeights.rbegin() < w){
        return vector<KeyFrame*>();
    }
    else{
        // 如果存在，返回满足要求的关键帧
        int n = it - mvOrderedConnectedWeights.begin();
        return vector<KeyFrame*>(mvpOrderedConnectedKeyFrames.begin(), mvpOrderedConnectedKeyFrames.begin()+n);
    }
}
 ...
 */
void KeyFrame::UpdateConnectedKeyFrameAndWeights(){
    ...
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend;){
        ...
        if(mit->second >= mnMinConnectedWeight){
...

    {
        ...
        // 更新当前帧与其它关键帧的连接权重
        mmConnectedKeyFrameandWeights = KFcounter;
        mvpOrderedConnectedKeyFrames = vector<KeyFrame*>(lKFs.begin(),lKFs.end());
        mvOrderedConnectedWeights = vector<int>(lWs.begin(), lWs.end());
        //printf("%u--%u--%u\n",mmConnectedKeyFrameandWeights.size(),mvpOrderedConnectedKeyFrames.size(),mvOrderedConnectedWeights.size());
        // Step 5 更新生成树的连接
...
{   
...

    {
        ...
        // 清空自己与其它关键帧之间的联系
        mmConnectedKeyFrameandWeights.clear();
        mvpOrderedConnectedKeyFrames.clear();

        // UpdateImgKPMPState Spanning Tree
        ...
        while(!mspChilds.empty())
        {
            ...
            for(set<KeyFrame*>::iterator sit=mspChilds.begin(), send=mspChilds.end(); sit != send; sit++){
                KeyFrame* pKF = *sit;
                // 跳过无效的子关键帧
                if(pKF->isBad())    
                    continue;
        ...
        if(!mspChilds.empty())
            for(set<KeyFrame*>::iterator sit=mspChilds.begin(); sit != mspChilds.end(); sit++)
            {
    ...
    mpKeyFrameDB->eraseKFromDB(this);
}
...
void KeyFrame::EraseConnection(KeyFrame* pKF)
{
    ...
    bool bUpdate = false;
    {
        unique_lock<mutex> lock(mMutexConnections);
        if(mmConnectedKeyFrameandWeights.count(pKF)){
            mmConnectedKeyFrameandWeights.erase(pKF);
            bUpdate=true;
        }
    }
