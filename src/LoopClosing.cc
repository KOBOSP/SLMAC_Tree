 ...
 */
    bool LoopClosing::ComputeSim3() {
        ...
        for (int i = 0; i < nInitialCandidates; i++) {
            ...
            int nmatches = matcher.SearchMatchKFAndKFByBoW(mpCurrentKF, pKF, vvpMapPointMatches[i]);
        ...
        while (nCandidates > 0 && !bMatch) {
            ...
            for (int i = 0; i < nInitialCandidates; i++) {
                ...
                // 如果计算出了Sim3变换，继续匹配出更多点并优化。因为之前 SearchMatchFrameAndKFByBoW 匹配可能会有遗漏
                if (!Scm.empty()) {
                    ...
                    // 只有互相都成功匹配的才认为是可靠的匹配
                    matcher.SearchMatchKFAndKFByProject(mpCurrentKF, pKF, vpMapPointMatches, s, R, t, 7.5);
        ...
        // 搜索范围系数为10
        matcher.SearchMatchKFAndMPsByProject(mpCurrentKF, mScw, mvpLoopMapPoints, mvpCurrentMatchedPoints, 10);
 ...
 */
    void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap) {
        ...
        for (KeyFrameAndPose::const_iterator mit = CorrectedPosesMap.begin(), mend = CorrectedPosesMap.end();
            ...
            // 搜索区域系数为4
            matcher.SearchReplaceKFAndMPsByProjectInGlobalMap(pKF, cvScw, mvpLoopMapPoints, 4, vpReplacePoints);

            // Get Map Mutex
            // 之所以不在上面 SearchReplaceKFAndMPsByProjectInLocalMap 函数中进行地图点融合更新的原因是需要对地图加锁
            unique_lock<mutex> lock(mpMap->mMutexMapUpdate);
