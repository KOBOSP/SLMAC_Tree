...
Frame::Frame(const Frame &frame)
    : mpORBvocabulary(frame.mpORBvocabulary),
     ...
     mDistCoef(frame.mDistCoef.clone()),					//深拷贝
      ...
      mnKeyPointNum(frame.mnKeyPointNum),
...
//    std::cout << mvKeys.size() <<","<<spent.count() << " sec \n";
    ...
    UndistortKeyPoints();
    // 初始化本帧的地图点
    mvpMapPoints = vector<MapPoint*>(mnKeyPointNum, static_cast<MapPoint*>(NULL));
...
void Frame::AssignFeaturesToGrid()
{
    ...
    int nReserve = mnKeyPointNum / (FRAME_GRID_COLS * FRAME_GRID_ROWS);
...
vector<size_t> Frame::GetFeaturesInArea(const float &x, const float  &y, const float  &r, bool NeedTarget, int minLevel, int maxLevel) const
{
    ...
    if(maxLevel>mnScaleLevels){
        maxLevel=mnScaleLevels;
    }
    const int r_2=r*r;
 ...
 */
bool Frame::IsPosInGrid(const cv::KeyPoint &kp, int &posX, int &posY)
{
...
void Frame::UndistortKeyPoints()
{
    ...
    cv::Mat mat(mnKeyPointNum, 2, CV_32F);
