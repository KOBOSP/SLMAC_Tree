...
public:
   ...
   */
    KeyFrameDatabase(const ORBVocabulary &voc, const string &strSettingPath);

  /**
   * @brief 根据关键帧的词包，更新数据库的倒排索引
   * @param pKF 关键帧
   */
   void addKFtoDB(KeyFrame* pKF);

  /**
   * @brief 关键帧被删除后，更新数据库的倒排索引
   * @param pKF 关键帧
   */
   void eraseKFromDB(KeyFrame* pKF);
   ...
   */
   std::vector<KeyFrame *> DetectLoopClosingCandidates(KeyFrame* pKF, float minScore);
...
protected:
  ...
  // Inverted file
  // 倒排索引，mvKFListHasWord[i]表示包含了第i个word id的所有关键帧
  std::vector<list<KeyFrame*> > mvKFListHasWord;
  float mfradioCommonWord,mfradioAccScore;
