...
private:
     ...
     */
    void GetHomographyMatrix(vector<bool> &vbMatchesInliers, float &MatchesScore, cv::Mat &H21);
     ...
     */
    void GetFundamentalMatrix(vector<bool> &vbInliers, float &score, cv::Mat &F21);
     ...
     * @return float                        返回得分
     */
    float GetHomographyScore(const cv::Mat &H21, const cv::Mat &H12, vector<bool> &vbMatchesInliers, float sigma);
     ...
     * @return float                        返回得分
     */
    float GetFundamentalScore(const cv::Mat &F21, vector<bool> &vbMatchesInliers, float sigma);
     ...
     * @return false                        初始化失败
     */
    bool ReconstructFromF(vector<bool> &vbMatchesInliers,
                          cv::Mat &F21, cv::Mat &K,
                          cv::Mat &R21,
                          cv::Mat &t21,
                          vector<cv::Point3f> &vP3D,
                          vector<bool> &vbTriangulated,
                          float minTriangulAngle,
                          int minTriangulatedMapPoint);
     ...
     * @return false                        初始化失败
     */
    bool ReconstructFromH(vector<bool> &vbMatchesInliers,
                          cv::Mat &H21,
                          cv::Mat &K,
                          cv::Mat &R21,
                          cv::Mat &t21,
     ...
     */
    void GetTriangulMapPoint(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &P1, const cv::Mat &P2, cv::Mat &x3D);
     ...
     */
    void NormalizePointCoord(const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T);
     ...
     * @param[out]  TriangulAngle			计算出来的比较大的视差角（注意不是最大，这个要看后面中程序的注释）
     * @return	int 返回本组解中good点的数目
     */
    int GetTriangulFromRT(const cv::Mat &R, const cv::Mat &t, const vector<cv::KeyPoint> &vKeys1, const vector<cv::KeyPoint> &vKeys2,
